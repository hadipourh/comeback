...
UINT64 boomerang(int R, int N3, u8 *dp, u8 *dc)
{
    ...
    for(int i = 0; i < 16; i++) dc[i] = dc[i] & 0xf;
    u16 k[(KSIZE/4)];
    u8 subkeys[R][8];
    u8 p1[16],p2[16];
    u8 c3[16],c4[16];
	...
	for (int t = 0; t < N3; t++){
        ...
        // Derive c3 and c4
        for(int i = 0; i < 16; i++) c3[i] = p1[i] ^ dc[i];
        for(int i = 0; i < 16; i++) c4[i] = p2[i] ^ dc[i];
...
int main(int argc, char **argv)
{
    ...
    fprintf(fic, "Boomerang distinguisher for %d rounds of TWINE\n", NUMBER_OF_ROUNDS);
